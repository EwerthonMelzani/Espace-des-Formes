...
\author{Final Presentation (Version 2)}
\date{\today}
...
\begin{itemize}
  ...
  \item \textbf{Crucial Result:} This breaks the rotational invariance of scalar kernels and massively increases the topological expressivity of the model. \textit{It is a structural necessity.}
\end{itemize}
\end{frame}
...
\textit{This completely reduces the continuous setting to an exact \textbf{finite-dimensional optimal control problem} over variables $a, g,$ and $\theta$.}
\end{frame}
...
\vspace{2mm}
\textbf{Role in practice:} PMP mechanics naturally provide the exact \textit{Backpropagation-Through-Time (BPTT)} equations needed to iteratively extract gradients.
\end{frame}
...
\centering \textit{This explicitly establishes mathematically rigorous backpropagation through the geometric deformation!}
\end{frame}
...
\begin{itemize}
  \item Datasets: circles, moons, blobs, gaussian quantiles.
  \item 2D baseline vs 3D lifting (The Dummy Dimension approach, injected with Gaussian noise $\sigma=0.01$).
  \item Paired seeds: 0, 1, 2.
  \item Metrics: test accuracy (mean and std).
\end{itemize}
\end{frame}
...
\begin{frame}{Who Performs Best Where? (Final Accuracy Criterion)}
\small
\centering
...
\vspace{3mm}
\begin{itemize}
  \item \textbf{Circles}: Strong topological barrier. Diffeo easily handles this via non-linear bending.
  \item \textbf{Gaussian Quantiles}: The Dummy 3D dimension is mathematically necessary to let the algorithm flow inner quantiles over the outer quantiles. Result: a $+7.2\%$ leap in performance exactly mimicking theory.
\end{itemize}
\end{frame}


\begin{frame}{Dynamic Deformations: 3D Dummy Lifting Animation}
\centering
% Extracted 49 frames for 3D
\animategraphics[loop,autoplay,width=0.6\textwidth]{10}{animacoes/3D_frames/frame_}{0}{48}

\vspace{1mm}
\footnotesize
This animation shows the exact trajectory of points escaping their 2D topological bind by lifting into 3D.
\end{frame}

\begin{frame}{Dynamic Deformations: 2D Target Flow Animation}
\centering
% Extracted 49 frames for 2D
\animategraphics[loop,autoplay,width=0.6\textwidth]{10}{animacoes/2D_frames/frame_}{0}{48}

\vspace{1mm}
\footnotesize
2D spatial deformations stretching clusters towards linear separability.
\end{frame}

\begin{frame}{Mathematical Topology Breaking (Section 5.1)}
\centering
% Extracted 61 frames for section5_1
\animategraphics[loop,autoplay,width=0.7\textwidth]{12}{animacoes/section5_1_frames/frame_}{0}{60}

\vspace{1mm}
\footnotesize
A direct visual proof of Section 5.1 from Younes (2019): escaping a locked ring using the continuous third dimension.
\end{frame}
